            ...
            <div className="relative w-full overflow-hidden">
              <div className="flex">
                {[...Array(3)].map((_, setIndex) => (
                  <div key={setIndex} className="flex gap-12 flex-shrink-0 animate-marquee">
                    {clientLogos.map((logo, index) => (
                      <div
                        key={`${setIndex}-${index}`}
                        className="flex items-center justify-center h-16 w-40 flex-shrink-0 hover:scale-110 transition-transform duration-300"
                      >
                        <img
                          src={logo.src}
                          alt={logo.alt}
